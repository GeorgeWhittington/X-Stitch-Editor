...
public:
    ...
    bool pan_camera(Vector2f delta, float position[3*4]);
    bool zoom_camera(float zoom, float position[3*4]);
    void zoom_to_point(Vector2f point, float zoom, float position[3*4]);

    float get_scale() { return _scale; };

    nanogui::Matrix4f generate_nanogui_matrix(Vector2f translation, float scale, float device_ratio);
    glm::mat4 generate_glm_matrix(Vector2f translation, float scale, float device_ratio);
    ...
    */
    nanogui::Vector4f translated_canvas_bounds(float position[3*4], Vector2f translation, float zoom);
    ...
    */
    Vector2f screen_to_ndc(Vector2i screen_coords);
    /* Returns the normalized device coordinates of a specified point on
    ...
    */
    Vector2f screen_to_ortho_ndc(Vector2i screen_coords);
    // Scales device size by the pixel_ratio of the screen (to handle apple retina screens)
    Vector2i get_scaled_device_dimensions();
    bool device_coords_in_bounds(Vector2i screen_coords);
    // Determine if an ndc coordinate is within the canvas bounds provided
    bool inside_canvas(Vector2f coords, Vector4f bounds);
